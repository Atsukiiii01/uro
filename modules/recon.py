...
class ReconEngine:
    ...
    def fetch_from_crtsh(self) -> Set[str]:
        """Queries crt.sh for passive subdomain discovery via CT logs."""
        logging.info(f"Querying crt.sh for {self.domain}...")
        url = f"https://crt.sh/?q=%.{self.domain}&output=json"
        subdomains = set()
        
        try:
            response = requests.get(url, headers={'User-Agent': 'uro-bugbounty-os'}, timeout=30)
            if response.status_code == 200:
                ...
                for entry in data:
                    ...
                    for name in name_value.split('\n'):
                        ...
                        # Clean wildcards and ensure it belongs to the target domain
            ...
            logging.error(f"Failed fetching from crt.sh: {str(e)}")
            
        return subdomains

    def fetch_from_subfinder(self) -> Set[str]:
        """Runs subfinder locally if installed in the system PATH."""
        subdomains = set()
        ...
        try:
            # Run subfinder silently and output JSON format
    ...
    def run_all(self) -> Set[str]:
        """Combines all passive sources for maximum structural coverage."""
        crt_subs = self.fetch_from_crtsh()
        sf_subs = self.fetch_from_subfinder()
        
        all_subs = crt_subs.union(sf_subs)
